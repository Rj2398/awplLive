...
const PendingPrescpt = ({ id }) => {
  ...
  // console.log(medicineSearchResults, "HARDCODED medicineSearchResults");

  useEffect(() => {
    dispatch(medicineSearch()); // Keep commented out if you're using hardcoded data
  }, [dispatch]);

  // console.log(id, "is appointment id ", allMedicine, "SELECTED DATA *******");
  ...
  // console.log(adviceText, "advice text ***");
  ...
  const currentChars = adviceText.length;
  ...
  // --- MODIFIED handleSubmitToAPI ---
  // This function now just opens the confirmation modal
  const handleSubmitToAPI = async () => {
    // Perform initial validation BEFORE showing the confirmation modal
    if (
      !data ||
      !data.disease_name ||
      !data.treatments ||
  ...
  // --- NEW handleConfirmSubmit FUNCTION ---
  // This function will actually send the data to the API
  const handleConfirmSubmit = async () => {
    setShowSubmitConfirmModal(false); // Close the confirmation modal
    setIsLoading(true); // Start loading when actually submitting
    ...
    try {
      const response = await axios.post(
        ...
        // `${baseUrl}api/doctor/prescriptions/submit_appointment_prescription`,
        `${baseUrl}api/doctor/prescriptions/submit_appointment_prescription`,
        payload,
        {
          headers: {
    ...
    <div className="mx-auto font-sans bg-white">
      ...
      {hasTreatments && (
        <div className="mt-6 text-center">
          {/* Changed onClick to show the confirmation modal */}
